...
static void get_ndt_param(ros::NodeHandle& nh)
{
  double init_x_param = 0;
  double init_y_param = 0;
  double init_z_param = 0;
  double init_roll_param = 0;
  double init_pitch_param = 0;
  double init_yaw_param = 0;
  int maximum_iteration_param = 30;
  float resolution_param = 1;
  double step_size_param = 0.1;
  double transformation_epsilon_param = 0.01;
  int use_gnss_param = 0;

  nh.param("init_x", init_x_param);
  nh.param("init_y", init_y_param);
  nh.param("init_z", init_z_param);
  nh.param("init_roll", init_roll_param);
  nh.param("init_pitch", init_pitch_param);
  nh.param("init_yaw", init_yaw_param);
  nh.param("maximum_iteration", maximum_iteration_param);
  nh.param("resolution", resolution_param);
  nh.param("step_size", step_size_param);
  nh.param("transformation_epsilon", transformation_epsilon_param);
  nh.param("use_gnss", use_gnss_param);

  if (_use_gnss != use_gnss_param)
  {
    init_pos_set = 0;
  }
  else if (_use_gnss == 0 &&
           (initial_pose.x != init_x_param || initial_pose.y != init_y_param || initial_pose.z != init_z_param ||
            initial_pose.roll != init_roll_param || initial_pose.pitch != init_pitch_param || initial_pose.yaw != init_yaw_param))
  {
    init_pos_set = 0;
  }

  _use_gnss = use_gnss_param;

  // Setting parameters
  if (resolution_param != ndt_res)
  {
    ndt_res = resolution_param;
  ...
  if (transformation_epsilon_param != trans_eps)
  {
    trans_eps = transformation_epsilon_param;
  ...
  if (maximum_iteration_param != max_iter)
  {
    max_iter = maximum_iteration_param;
  ...
  {
    initial_pose.x = init_x_param;
    initial_pose.y = init_y_param;
    initial_pose.z = init_z_param;
    initial_pose.roll = init_roll_param;
    initial_pose.pitch = init_pitch_param;
    initial_pose.yaw = init_yaw_param;

    if (_use_local_transform == true)
    {
      tf::Vector3 v(init_x_param, init_y_param, init_z_param);
      tf::Quaternion q;
      q.setRPY(init_roll_param, init_pitch_param, init_yaw_param);
...
{
  ros::init(argc, argv, "ndt_matching_param");
  ...
  // Subscribers
  // ros::Subscriber param_sub = nh.subscribe("config/ndt", 10, param_callback); # Modified by Hayeon Park
